...
# ============================================================================
# OCR Utilities (Migrated from ocr_handler.py)
# ============================================================================
import base64
import tempfile
...
if CLOUDFLARE_API_TOKEN:
    try:
        ...
        print(f"⚠️ Failed to initialize Cloudflare OCR client: {e}")
else:
    print("⚠️ CLOUDFLARE_API_TOKEN not set. Cloudflare OCR will be skipped.")


# Initialize Google SDK for Gemma 4 fallback
GOOGLE_API_KEY_4 = os.getenv("GOOGLE_API_KEY_4")
if GOOGLE_API_KEY_4:
    genai.configure(api_key=GOOGLE_API_KEY_4)
else:
    print("⚠️ GOOGLE_API_KEY_4 not set. Gemma fallback OCR will not work.")

def _call_gemma_ocr_from_path(file_path: str) -> Dict[str, Any]:
    """Call Google Gemma 4 using Files API as a fallback."""
    try:
        ...
        _native_model = genai.GenerativeModel("gemma-4-31b-it")
        PROMPT = (
        ...
        response = _native_model.generate_content([PROMPT, file_obj])
        text_content = response.text
        
        try:
            json_str = extract_json_block(text_content)
            data = json.loads(json_str)
            text = data.get("latex_text", text_content)
        except:
            text = text_content
            
        print(f"[OCR] Gemma 4 fallback success: extracted {len(text)} characters")
        return {
            "text": text,
            "input_type": "image",
            "success": True
        ...
        print(f"[OCR] Gemma 4 fallback error: {e}")
        return {
...
def process_image_from_path(image_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Call Cloudflare OCR first
        cf_result = _call_cloudflare_ocr(image_data_uri)
        
        # Fallback condition check
        if not cf_result.get("success", False) or "No text detected" in cf_result.get("text", "") or "[Error" in cf_result.get("text", ""):
            print("[OCR] Cloudflare returned error/no text, falling back to Gemma 4...")
            if GOOGLE_API_KEY_4:
        ...
        print(f"[OCR] file-path OCR error: {e}")
        return {
...
def process_image_from_base64(base64_string: str) -> Dict[str, Any]:
    ...
    tmp_path = None
    try:
        # If it's already a data URI, attempt Cloudflare first and fallback to Gemma.
        if base64_string.startswith('data:image/'):
            cf_result = _call_cloudflare_ocr(base64_string)
            if not cf_result.get("success", False) or "No text detected" in cf_result.get("text", "") or "[Error" in cf_result.get("text", ""):
                print("[OCR] Cloudflare failed for base64 image, falling back to Gemma 4...")
                if GOOGLE_API_KEY_4:
        ...
        # Process from file (includes fallback behavior)
        result = process_image_from_path(tmp_path)
        return result
        
    except Exception as e:
        print(f"[OCR] base64 OCR error: {e}")
        return {
...
def _call_cloudflare_ocr(image_data_uri: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[OCR] Cloudflare error: {e}")
        return {
